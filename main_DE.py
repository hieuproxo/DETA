...
def run():
	...
	if opt.model == 'BPR':
		student_model = BPR_DE(user_count, item_count, teacher_user_emb, teacher_item_emb, student_dim, num_experts, gpu)
	elif opt.model == 'NeuMF':
		student_model = NeuMF_DE(user_count, item_count, teacher_user_MF, teacher_item_MF, teacher_user_MLP, teacher_item_MLP, student_dim, num_layers, num_experts, gpu)
	elif opt.model == 'LightGCN':
		student_model = LightGCN_DE(user_count, item_count, teacher_user_emb, teacher_item_emb, student_dim, num_layers, Graph, num_experts, gpu)
	else:
		assert False
	...
	DE_run(opt, student_model, gpu, optimizer, train_loader, test_dataset, model_save_path=opt.saved_models + dataset + '/' + opt.model + '_DE_' + str(student_dim) + '_seed_' + str(seed))
