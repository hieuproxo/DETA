...
def run():
	...
	if opt.model == 'BPR':
		...
		model = NeuMF(user_count, item_count, dim, num_layers, gpu)
	elif opt.model == 'LightGCN':
		dim = opt.dim
		num_layers = opt.num_layers
		Graph = gen_graph.getSparseGraph(train_mat, user_count, item_count, gpu)
		model = LightGCN(user_count, item_count, dim, num_layers, Graph, gpu)
	else:
		assert False
	model = model.to(gpu)
	...
	no_KD_run(opt, model, gpu, optimizer, train_loader, test_dataset, model_save_path=opt.saved_models + dataset + '/' + opt.model + '_' + str(dim) + '_seed_' + str(seed))
